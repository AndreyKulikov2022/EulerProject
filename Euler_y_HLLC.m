...
a=sqrt(gamma*p./ro);
%% Step1, Toro p.332
p_s=1/2*(p(:,1:end-1)+p(:,2:end))-1/2*(v(:,2:end)-v(:,1:end-1)).*...
...
q_L(p_s<=p(:,1:end-1))=1;
S_L=v(:,1:end-1)-a(:,1:end-1).*q_L;
q_R=sqrt(...
    1+(gamma+1)/(2*gamma)*(p_s./p(:,2:end)-1)...
    );
q_R(p_s<=p(:,2:end))=1;
S_R=v(:,2:end)+a(:,2:end).*q_R;
S=(p(:,2:end)-p(:,1:end-1)+ro(:,1:end-1).*v(:,1:end-1).*(S_L-v(:,1:end-1))-ro(:,2:end).*v(:,2:end).*(S_R-v(:,2:end)))./...
    (ro(:,1:end-1).*(S_L-v(:,1:end-1))-ro(:,2:end).*(S_R-v(:,2:end)));
%% Step3
P_LR=1/2*(p(:,2:end)+p(:,1:end-1)+ro(:,1:end-1).*(S_L-v(:,1:end-1)).*(S-v(:,1:end-1))+...
    ro(:,2:end).*(S_R-v(:,2:end)).*(S-v(:,2:end)));
SL_less=S_L<0;
S_less=S<0;
SR_less=S_R<0;
...
F_rou_sL=S.*(S_L.*rou(:,1:end-1)-f_rou(:,1:end-1))./(S_L-S);
F_rov_sL=(S.*(S_L.*rov(:,1:end-1)-f_rov(:,1:end-1))+S_L.*P_LR)./(S_L-S);
F_E_sL=(S.*(S_L.*E(:,1:end-1)-f_E(:,1:end-1))+S_L.*P_LR.*S)./(S_L-S);
F_ro_sR=S.*(S_R.*ro(:,2:end)-f_ro(:,2:end))./(S_R-S);
F_rou_sR=S.*(S_R.*rou(:,2:end)-f_rou(:,2:end))./(S_R-S);
F_rov_sR=(S.*(S_R.*rov(:,2:end)-f_rov(:,2:end))+S_R.*P_LR)./(S_R-S);
